	...
	Face *buildingFaces, QuadrangleRayTube *&reflectedRayTubes)
{
	...
	GetReflectionWithBuildingFace(incidentRayTubes, buildingFaceCnt, RayTubeCnt, buildingFaces, tmp_reflectedRayTubes, isReflectionExist);
	...
	const int incidentRayTubeCnt, const int faceCnt, int &reflectedRayTubeCnt)
{
	...
	CalculateTriangleReflectedRayTubes(incidentRayTubeCnt, faceCnt, faces, incidentRayTubes, tmp_reflectedRayTubes, isReflectedTubeExist, tmp_distance);//此处调用求三角形射线管与地面反射GPU函数~~~~~~~~~~~
