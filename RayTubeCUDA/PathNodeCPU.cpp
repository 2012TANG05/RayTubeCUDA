	...
	const int rayTubeCount, int &receivedRayTubeCnt)
{
	...
	IsTriangleRayTubesReceived(rayTubes, receiver, rayTubeCount, isReceived);//CPU函数
	...
	const int rayTubeCount, int &receivedRayTubeCnt)
{
	...
	bool *isReceived = new bool[rayTubeCount];
	IsQuadrangleRayTubesReceived(rayTubes, receiver, rayTubeCount, isReceived);//CPU函数
	receivedRayTubeCnt = GetReceivedRayTubeCount(isReceived, rayTubeCount);
	...
	const int incidentRayTubeCnt, const int faceCnt, int &reflectedRayTubeCnt)
{
	...
	CPUCalculateTriangleReflectedRayTubes(incidentRayTubeCnt, faceCnt, faces, incidentRayTubes, tmp_reflectedRayTubes, isReflectedTubeExist, tmp_distance);//此处调用求三角形射线管与地面反射GPU函数~~~~~~~~~~~
	int i,j = 0;
	for(i = 0; i<20480;i++)
	{
