...
__device__ void RADC_UitizeVectorOnGPU(float *a,float *b,float *c)//单位化向量
{
...
__device__ Point RADC_GetReflectedVectorOnGPU(Point d,Point n)//计算反射射线方向向量
{
...
__device__ Ray RADC_CalculateReflectedRayOnGPU(Ray incidentRay,Face *face,int faceCount,int *reflectedFace,int *flag)
{
	Ray reflectedRay;
	Point defaultPoint;
		...
		if (abs(denominator)>0.000001)
		{
			...
			if(t_numerator/denominator<t&&t_numerator/denominator>1&&u>=0&&u<=1&&v>0&&(u+v)<1&&v<=1&&remain<8)
			{
				...
				*reflectedFace=i;
				...
				RADC_UitizeVectorOnGPU(&n.x,&n.y,&n.z);
				reflectedRay.direction=RADC_GetReflectedVectorOnGPU(incidentRay.direction,n);
			}
		}
	}
	return reflectedRay;
...
__device__ Ray RADC_CalculateVirtualReflectedRayOnCPU(Ray incidentRay,Face face)
{
	Ray reflectedRay;
	Point defaultPoint;
	defaultPoint.x=0;defaultPoint.y=0;defaultPoint.z=0;
...
__device__ bool JudgeTwoTriangleFacesOnTheSameQuadrangleOnGPU(int faceNumber1,int faceNumber2)
{
...
__device__ bool JudgeTwoTriangleFacesOnTheAdjacentQuadrangleOnGPU(int faceNumber1,int faceNumber2)
{
...
__device__ int GetRemainingOneNumberOnGPU(int a,int b)
{
	return 3-(a+b);
}

__device__ void GetRemainingTwoNumberOnGPU(int a,int *b,int *c)
{
	if (a==0)
	{
		*b=1;
...
__device__ void AddPathNodeToReflectedRayTube(TriangleRayTube *incidentRayTubes,TriangleRayTube *reflectedRayTubes,int i,Face face1,Face face2)
{
...
__device__ void GetReflectedRayTubeWhenOneReflectedRayExist(int flagNumber,Ray *incidentRays,TriangleRayTube *reflectedRayTubes,Ray *reflectedRays,int *reflectedFace,Face *face,int i,int *reflectionExist,int *diffractionExist,int *faceNumber1,int *faceNumber2)
{
	...
	GetRemainingTwoNumberOnGPU(flagNumber,&remainingNumber1,&remainingNumber2);
	reflectedRayTubes[2*i].ray1=reflectedRays[flagNumber];
	reflectedRayTubes[2*i].ray2=RADC_CalculateVirtualReflectedRayOnCPU(incidentRays[remainingNumber1],face[reflectedFace[flagNumber]]);
	reflectedRayTubes[2*i].ray3=RADC_CalculateVirtualReflectedRayOnCPU(incidentRays[remainingNumber2],face[reflectedFace[flagNumber]]);
	reflectedRayTubes[2*i+1].ray1=reflectedRays[flagNumber];
	reflectedRayTubes[2*i+1].ray2=RADC_CalculateVirtualReflectedRayOnCPU(incidentRays[remainingNumber1],face[reflectedFace[flagNumber]]);
	reflectedRayTubes[2*i+1].ray3=RADC_CalculateVirtualReflectedRayOnCPU(incidentRays[remainingNumber2],face[reflectedFace[flagNumber]]);
...
{
	int remainingNumber=GetRemainingOneNumberOnGPU(flagNumber1,flagNumber2);
	reflectedRayTubes[2*i].ray1=reflectedRays[flagNumber1];
	reflectedRayTubes[2*i].ray2=RADC_CalculateVirtualReflectedRayOnCPU(incidentRays[flagNumber2],face[reflectedFace[flagNumber1]]);
	reflectedRayTubes[2*i].ray3=RADC_CalculateVirtualReflectedRayOnCPU(incidentRays[remainingNumber],face[reflectedFace[flagNumber1]]);
	reflectedRayTubes[2*i+1].ray1=reflectedRays[flagNumber2];
	reflectedRayTubes[2*i+1].ray2=RADC_CalculateVirtualReflectedRayOnCPU(incidentRays[flagNumber1],face[reflectedFace[flagNumber2]]);
	reflectedRayTubes[2*i+1].ray3=RADC_CalculateVirtualReflectedRayOnCPU(incidentRays[remainingNumber],face[reflectedFace[flagNumber2]]);
	if (JudgeTwoTriangleFacesOnTheAdjacentQuadrangleOnGPU(reflectedFace[flagNumber1],reflectedFace[flagNumber2]))
	{
...
__device__ void GetReflectedRayTubeWhenThreeReflectedRayExist(int flagNumber1,int flagNumber2,Ray *incidentRays,TriangleRayTube *reflectedRayTubes,Ray *reflectedRays,int *reflectedFace,Face *face,int i,int *reflectionExist,int *diffractionExist,int *faceNumber1,int *faceNumber2)
{
	...
	reflectedRayTubes[2*i].ray3=RADC_CalculateVirtualReflectedRayOnCPU(incidentRays[remainingNumber],face[reflectedFace[flagNumber1]]);
	reflectedRayTubes[2*i+1].ray1=RADC_CalculateVirtualReflectedRayOnCPU(incidentRays[flagNumber1],face[reflectedFace[remainingNumber]]);
	reflectedRayTubes[2*i+1].ray2=RADC_CalculateVirtualReflectedRayOnCPU(incidentRays[flagNumber2],face[reflectedFace[remainingNumber]]);
	reflectedRayTubes[2*i+1].ray3=reflectedRays[remainingNumber];
	diffractionExist[i]=1;
	reflectionExist[i]=1;
	if (JudgeTwoTriangleFacesOnTheAdjacentQuadrangleOnGPU(reflectedFace[remainingNumber],reflectedFace[flagNumber1]))
	{
		faceNumber1[i]=reflectedFace[flagNumber1];
	}
	else
...
__device__ void AntiSortPointsOfEdge(Edge *edge)
{
	Point tempPoint;
	if ((*edge).p1.z<(*edge).p2.z)
	{
...
__device__ Edge GetDiffractionEdgeWhenTwoReflectedRaysExist(Face face1,Face face2,Point reflectionPoint1,Point reflectionPoint2,Point VitualReflectionPoint,int *diffractionExist)
{
	...
	AntiSortPointsOfEdge(&diffractionEdge);
...
__global__ void CalculateReflectionAndDiffractionWithBuildingFace(TriangleRayTube *incidentRayTubes,int faceCount,int rayTubeCount,TriangleRayTube *reflectedRayTubes,Face *face,Edge *diffractionEdge,int *faceNumber1,int *faceNumber2,int *reflectionExist,int *diffractionExist)
{
	...
	reflectedRays[0]=RADC_CalculateReflectedRayOnGPU(incidentRays[0],face,faceCount,&reflectedFace[0],&flag[0]);
	reflectedRays[1]=RADC_CalculateReflectedRayOnGPU(incidentRays[1],face,faceCount,&reflectedFace[1],&flag[1]);
	reflectedRays[2]=RADC_CalculateReflectedRayOnGPU(incidentRays[2],face,faceCount,&reflectedFace[2],&flag[2]);
	difference=RADC_GetMaxNumber(reflectedFace[0],reflectedFace[1],reflectedFace[2])-RADC_GetMinNumber(reflectedFace[0],reflectedFace[1],reflectedFace[2]);
	...
	else
	{
		if (flag[0]+flag[1]+flag[2]==1)
		{
			...
			for (int j=0;j<3;j++)
			{
				if (flag[j]==1)
				{
					...
					AddPathNodeToReflectedRayTube(incidentRayTubes,reflectedRayTubes,i,face[reflectedFace[j]],face[reflectedFace[j]]);
					reflectionExist[i]=1;
					diffractionExist[i]=0;
				}
			}
		...
		if (flag[0]+flag[1]+flag[2]==2)
		{
			if (flag[0]==0)
			{
				AddPathNodeToReflectedRayTube(incidentRayTubes,reflectedRayTubes,i,face[reflectedFace[1]],face[reflectedFace[2]]);
				if (JudgeTwoTriangleFacesOnTheSameQuadrangleOnGPU(reflectedFace[1],reflectedFace[2]))
				{
				...
				else
				{
					...
					//AddPathNodeToReflectedRayTube(incidentRayTubes,reflectedRayTubes,i,face[reflectedFace[1]],face[reflectedFace[2]]);
					if (JudgeTwoTriangleFacesOnTheAdjacentQuadrangleOnGPU(reflectedFace[1],reflectedFace[2]))
					{
						diffractionEdge[i]=GetDiffractionEdgeWhenTwoReflectedRaysExist(face[reflectedFace[1]],face[reflectedFace[2]],reflectedRays[1].originalPoint,reflectedRays[2].originalPoint,reflectedRays[0].originalPoint,&diffractionExist[i]);
					}
				}
			}
		  if (flag[1]==0)
			{
				AddPathNodeToReflectedRayTube(incidentRayTubes,reflectedRayTubes,i,face[reflectedFace[0]],face[reflectedFace[2]]);
				if (JudgeTwoTriangleFacesOnTheSameQuadrangleOnGPU(reflectedFace[0],reflectedFace[2]))
				{
				...
				else
				{
					...
					if (JudgeTwoTriangleFacesOnTheAdjacentQuadrangleOnGPU(reflectedFace[0],reflectedFace[2]))
					{
						diffractionEdge[i]=GetDiffractionEdgeWhenTwoReflectedRaysExist(face[reflectedFace[0]],face[reflectedFace[2]],reflectedRays[0].originalPoint,reflectedRays[2].originalPoint,reflectedRays[1].originalPoint,&diffractionExist[i]);
					}
				}
			...
			if (flag[2]==0)
			{
				AddPathNodeToReflectedRayTube(incidentRayTubes,reflectedRayTubes,i,face[reflectedFace[0]],face[reflectedFace[1]]);
				if (JudgeTwoTriangleFacesOnTheSameQuadrangleOnGPU(reflectedFace[0],reflectedFace[1]))
				{
					GetReflectedRayTubeWhenOneReflectedRayExist(0,incidentRays,reflectedRayTubes,reflectedRays,reflectedFace,face,i,reflectionExist,diffractionExist,faceNumber1,faceNumber2);
				}
				else
				{
					...
					if (JudgeTwoTriangleFacesOnTheAdjacentQuadrangleOnGPU(reflectedFace[0],reflectedFace[1]))
					{
						diffractionEdge[i]=GetDiffractionEdgeWhenTwoReflectedRaysExist(face[reflectedFace[0]],face[reflectedFace[1]],reflectedRays[0].originalPoint,reflectedRays[1].originalPoint,reflectedRays[2].originalPoint,&diffractionExist[i]);
					}
				}			
			}
		}
		...
		{
			if (JudgeTwoTriangleFacesOnTheSameQuadrangleOnGPU(reflectedFace[0],reflectedFace[1])&&JudgeTwoTriangleFacesOnTheSameQuadrangleOnGPU(reflectedFace[0],reflectedFace[2]))
			{
			...
			{	
				if (JudgeTwoTriangleFacesOnTheSameQuadrangleOnGPU(reflectedFace[0],reflectedFace[1]))
				{
					...
					if(JudgeTwoTriangleFacesOnTheAdjacentQuadrangleOnGPU(reflectedFace[1],reflectedFace[2]))
					{				
						diffractionEdge[i]=GetDiffractionEdgeWhenThreeReflectedRaysExist(face[reflectedFace[1]],face[reflectedFace[2]],reflectedRays[0].originalPoint,reflectedRays[1].originalPoint,reflectedRays[2].originalPoint,&diffractionExist[i]);
					}
					else
					{
						diffractionEdge[i]=GetDiffractionEdgeWhenThreeReflectedRaysExist(face[reflectedFace[0]],face[reflectedFace[2]],reflectedRays[0].originalPoint,reflectedRays[1].originalPoint,reflectedRays[2].originalPoint,&diffractionExist[i]);
					}			
				}
				if (JudgeTwoTriangleFacesOnTheSameQuadrangleOnGPU(reflectedFace[0],reflectedFace[2]))
				{
					...
					if(JudgeTwoTriangleFacesOnTheAdjacentQuadrangleOnGPU(reflectedFace[0],reflectedFace[1]))
					{				
						diffractionEdge[i]=GetDiffractionEdgeWhenThreeReflectedRaysExist(face[reflectedFace[0]],face[reflectedFace[1]],reflectedRays[0].originalPoint,reflectedRays[2].originalPoint,reflectedRays[1].originalPoint,&diffractionExist[i]);
					}
					else
					{
						diffractionEdge[i]=GetDiffractionEdgeWhenThreeReflectedRaysExist(face[reflectedFace[1]],face[reflectedFace[2]],reflectedRays[0].originalPoint,reflectedRays[2].originalPoint,reflectedRays[1].originalPoint,&diffractionExist[i]);
					}		
				}
				if (JudgeTwoTriangleFacesOnTheSameQuadrangleOnGPU(reflectedFace[1],reflectedFace[2]))
				{
					AddPathNodeToReflectedRayTube(incidentRayTubes,reflectedRayTubes,i,face[reflectedFace[1]],face[reflectedFace[0]]);
					...
					if(JudgeTwoTriangleFacesOnTheAdjacentQuadrangleOnGPU(reflectedFace[0],reflectedFace[1]))
					{				
						diffractionEdge[i]=GetDiffractionEdgeWhenThreeReflectedRaysExist(face[reflectedFace[0]],face[reflectedFace[1]],reflectedRays[1].originalPoint,reflectedRays[2].originalPoint,reflectedRays[0].originalPoint,&diffractionExist[i]);
					}
					else
					{
						diffractionEdge[i]=GetDiffractionEdgeWhenThreeReflectedRaysExist(face[reflectedFace[0]],face[reflectedFace[2]],reflectedRays[1].originalPoint,reflectedRays[2].originalPoint,reflectedRays[0].originalPoint,&diffractionExist[i]);
					}		
				}
			}
		}
